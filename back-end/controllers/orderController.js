...
const deleteOrder = async (req, res) => {
  try {
    const { orderId, productId, size } = req.body;  // Ensure size is included in the request body
    ...
    // Initialize variables for canceled amount and updated items
    let canceledAmount = 0;
    const updatedItems = [];

    // Iterate through the items to remove the selected item based on productId and size
    for (const item of items) {
      if (item.product_id === productId && item.size === size) {
        canceledAmount += item.price * item.quantity; // Add price * quantity of the canceled item
      } else {
        updatedItems.push(item); // Keep the other items in the order
      }
    }
    ...
    // Update the total amount by subtracting the canceled amount
    order.total_amount -= canceledAmount;
